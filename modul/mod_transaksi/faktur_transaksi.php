...
$w = array(6, 40, 40, 40, 40 ,40); //163
//=========0, 1,  2,  3,  4,  5,  6, 7=====//
$pdf->setX(5);
$pdf->SetFont('courier', 'B', 8);
$pdf->Cell($w[1], 6, 'ID ORDER', 'TB', 0, 'C');
$pdf->Cell($w[2], 6, 'NAMA PEMBELI', 'TB', 0, 'C');
$pdf->Cell($w[3], 6, 'HARGA', 'TB', 0, 'C');
...
$pdf->Cell($w[2], 6, $data['nama_pembeli'], 'TB', 0, 'C');
$pdf->Cell($w[3], 6, format_money($data['harga']), 'TB', 0, 'C');
...
$pdf->Cell($w[3], 6, 'Kembalian', 'T', 0, 'C');
$pdf->Cell($w[4], 6, format_money($data['total_bayar'] - $data['harga'] ), 'T', 0, 'C');
$pdf->Ln();
