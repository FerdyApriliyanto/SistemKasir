    ...
    if ($_SESSION['hak_akses'] == 'guru') {
        if ($module == 'profil' AND $act == 'update') {
            ...
            mysqli_query($koneksi,"UPDATE guru SET foto_guru  = '$file'
                                WHERE nip = '$_SESSION[id_pengguna]'") or die(mysqli_error());
            header('location:../../index.php?modul=' . $module);
        } else if ($module == 'password' AND $act == 'update') {
            //QUERY PASSWORD LAMA
            $cek = mysqli_query($koneksi,"SELECT password FROM guru WHERE username = '$_SESSION[id_pengguna]'") or die(mysqli_error());
            ...
            if ($password1 != $password_lama) {
                ...
                mysqli_query($koneksi,"UPDATE guru SET password  = '$password3'
                                WHERE username      = '$_SESSION[id_pengguna]'") or die(mysqli_error());
        ...
        if ($module == 'profil' AND $act == 'update') {
            ...
            mysqli_query($koneksi,"UPDATE siswa SET foto_siswa  = '$file'
                                WHERE nis = '$_SESSION[id_pengguna]'") or die(mysqli_error());
            header('location:../../index.php?modul=' . $module);
        } else if ($module == 'password' AND $act == 'update') {
            //QUERY PASSWORD LAMA
        ...
        if ($module == 'profil' AND $act == 'update') {
            ...
            mysqli_query($koneksi,"UPDATE admin SET foto_admin  = '$file'
                                WHERE id_admin      = '$_SESSION[id_pengguna]'") or die(mysqli_error());
            header('location:../../administrator/index.php?modul=' . $module);
        }
    }
}
